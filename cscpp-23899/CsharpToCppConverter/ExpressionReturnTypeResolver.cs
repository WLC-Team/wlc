    ...
    public class ExpressionReturnTypeResolver
    {
        ...
        public IEnumerable<ResolvedContextItem> Resolve(LiteralExpression literalExpression, IEnumerable<ResolvedContextItem> resolvedContext = null)
        {
            ...
            if (resolvedContext == null)
            {
                ...
                //var firstunion = foundFoundReturnTypesOfMembersFromClassScope.Union(foundTypesOfReferencesFromGlobalNamespacesScope);

                //Phani-BugID0001-ApplicationHangingTyringToResolveFormVariables
                var secondunion = foundTypesOfVarsOrParamsFromLocalScope.Union(foundTypesOfReferencesFromGlobalNamespacesScope);

                return secondunion.ToList();
        ...
        private IEnumerable<ICodeElement> SelectBaseClassHierarchy(Class @class)
        {
            ...
            while (!string.IsNullOrWhiteSpace(current.BaseClass))
            {
                ...
                //Phani-BugID0001-AssertTryingToResolveClassTypesinFormClass
                //Debug.Assert(codeElements.Any(), string.Format("Could not resolve type: {0}", current.BaseClass));


                //Phani-BugID001-AssertTryingtoResolveClassTypeInFormClass
                if (codeElements.Count == 0 || codeElements == null)
                {
                    // return System.Object
                    var platformObjectTypeDef =
                    ...
                    yield return platformObjectTypeDef;
                }
